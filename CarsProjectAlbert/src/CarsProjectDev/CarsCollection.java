 ...
 *
 * PUBLIC FEATURES: // Constructors public CarsCollection() public
 * CarsCollection(Manufacturer man)
 *
 * // Methods public int addCar(Car c) public int carsCount() public int
 * manufacturerCount() public Car[] getAllCars() public Manufacturer[]
 * getAllManufacturers() public double getAverageAge() public double
 * getAverageDistance() public double getAveragePrice() public void
 * loadCars(String file) throws IOException, ClassNotFoundException public void
 * saveCars(String file) throws IOException public Car[] search(int minPrice,
 * int maxPrice, double minDistance, double maxDistance) public Car[] search(int
 * minAge, int maxAge)
 *
...
public class CarsCollection {
	/**
	 * this constant is returned by the addCar method to indicate the car was
	 * successfully added to the Car Sales System
	 */
	public static final int NO_ERROR = 0;
	/**
	 * this constant is returned by the addCar method to indicate the car wasn't
	 * successfully added to the Car Sales System because the manufacturer has
	 * reached it's maximum of 20 cars
	 */
	public static final int CARS_MAXIMUM_REACHED = 1;
	/**
	 * this constant is returned by the addCar method to indicate the car wasn't
	 * successfully added to the Car Sales System because the system has reached
	 * it's maximum of 20 manufacturers
	 */
	public static final int MANUFACTURERS_MAXIMUM_REACHED = 2;

	private static final int MAXMANUFACTURERS = 20;
	...
	/**
	 * adds a car to a CarCollection and files it in an appropriate manufacturer, or
	 * creates a new manufacturer if none exist for the car
	 *
	 * @param c car to add to collection
	 * @return either one of NO_ERROR, CARS_MAXIMUM_REACHED, or
	 *         MANUFACTURERS_MAXIMUM_REACHED
	 */
	 ...
	 *
	 * @return value indicating the average distance travelled of all the cars in
	 *         the collection
	 */
		...
		if (manufacturer.length > 0) {
			do {
				...
				for (int i = 0; i < items; i++) {
					if (i + 1 < items) {
						if (manufacturer[i].getManufacturerName()
								.compareTo(manufacturer[i + 1].getManufacturerName()) > 0) {
							temp = manufacturer[i];
	...
	public Car[] search(int minAge, int maxAge) {
		...
		/*
		 * Putting the if statement first will increase effeciency since it isn't
		 * constantly checking the condition for each loop. It does use almost double
		 * the amount of code though
		 */
		if (maxAge == -1) {
			for (int i = 0; i < car.length; i++) {
