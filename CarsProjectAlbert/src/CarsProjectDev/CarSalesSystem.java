 ...
 * collection. It also creates instances of the other panels (add car, view car,
 * etc) and incorporates them into the main frame. This is hierarchically, the
 * highest class. @
 *
 * PUBLIC FEATURES: // Constructors public CarSalesSystem(String f)
 *
 * // Methods public void aboutMenuItemClicked() public void
 * actionPerformed(ActionEvent ev) public void addCarUpdateListener(Object
 * listener)
 * 
 * public int addNewCar(Car c) public void closing() public void
 * componentHidden(ComponentEvent ev) public void componentMoved(ComponentEvent
 * ev) public void componentResized(ComponentEvent ev) public void
 * componentShown(ComponentEvent ev) public static double[]
 * convertToRange(String s) public Car[] getAllCars() public boolean
 * getCarsUpdated() public double getStatistics(int type) public static void
 * main(String[] args) public Car[] search(int minAge, int maxAge) public Car[]
 * search(int minPrice, int maxPrice, double minDistance, double maxDistance)
 * public void setCarsUpdated() public void stateChanged(ChangeEvent ev) public
 * static Car[] vectorToCar(Vector v)
 *
 * COLLABORATORS: AboutDialog, CarsCollection, WindowCloser, WelcomePanel,
 * AddCarPanel ShowAllCarsPanel, SearchByAgePanel, SearchByOtherPanel
 *
...
public class CarSalesSystem extends JFrame implements ActionListener, ComponentListener, ChangeListener {
	 ...
	 * you wish to find the total number of cars in the system
	 */
	public static final int CARS_COUNT = 0;
	/**
	 * is used as the parameter in the 'getStatistics(int type)' method to indicate
	 * you wish to find the total number of manufacturers in the system
	 */
	public static final int MANUFACTURERS_COUNT = 1;
	/**
	 * is used as the parameter in the 'getStatistics(int type)' method to indicate
	 * you wish to find the average price, from the entire collection of cars in the
	 * system
	 */
	public static final int AVERAGE_PRICE = 2;
	/**
	 * is used as the parameter in the 'getStatistics(int type)' method to indicate
	 * you wish to find the average distance travelled (in kilometers) from the
	 * entire collection of cars in the system
	 */
	public static final int AVERAGE_DISTANCE = 3;
	/**
	 * is used as the parameter in the 'getStatistics(int type)' method to indicate
	 * you wish to find the average age from the entire collection of cars in the
	 * system
	 */
	 ...
	 * get resized below a minimum size which could cause display problems in the
	 * application
	 *
	 ...
	 *         element indicating maximum bounds. A value of -1 for minimum and
	 *         maximum bounds indicates an error in converting the parameter to a
	 *         proper range. A value of -1 for the maximum bounds and a value > 0
	 *         for the minimum bounds indicates the maximum bounds is infinity in
	 *         theory. If both bounds are greater than 0, this indicates a valid
	 *         range a to b, where a and b can be equal.
	 */
	 ...
	 *             AVERAGE_DISTANCE or AVERAGE_AGE
	 ...
	 * also send messages to all registered car update listeners
	 */
